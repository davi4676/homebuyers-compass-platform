...
import { getCachedZipData, setCachedZipData } from '@/lib/zillow-cache'
import fs from 'fs'
import path from 'path'

/**
 * API Route to fetch Zillow ZHVI data for zip codes
 ...
 * 2. Fetches the latest Zip_zhvi_uc_sfrcondo_tier_0.33_0.67_sm_sa_month.csv if cache expired
 * 3. Finds the zip code
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Zillow Research Data URL for Zip Code ZHVI (All Homes)
    // This is the latest CSV file from Zillow Research Data
    const zillowDataUrl = 'https://files.zillowstatic.com/research/public_csvs/zhvi/Zip_zhvi_uc_sfrcondo_tier_0.33_0.67_sm_sa_month.csv'
    const localFileName = 'Zip_zhvi_uc_sfrcondo_tier_0.33_0.67_sm_sa_month.csv'
    const localFilePath = path.join(process.cwd(), 'data', 'zillow', localFileName)
    
    // Check cache first (unless force refresh)
    let csvText: string
    let cachedData = getCachedZipData()
    ...
    if (!forceRefresh && cachedData) {
      ...
      try {
        ...
        if (cachedData) {
          ...
          console.error('Error fetching Zillow zip data:', fetchError)
          throw fetchError
        }
      }
    }
    ...
    if (!cachedData || forceRefresh) {
      setCachedZipData(csvText, latestDate)
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const regionName = columns[2]?.trim() // RegionName column (zip code)
      const regionType = columns[3]?.trim() // RegionType column
      
      // Match zip code
      if (regionType === 'Zip' && regionName === zipCode) {
        const zhviValue = columns[5 + latestDateIndex]?.trim() // Data column for latest date
        
        if (zhviValue && !isNaN(parseFloat(zhviValue))) {
          // Try to get city and state from the data if available
          const stateName = columns[4]?.trim() || ''
    ...
    // Zip code not found
    return NextResponse.json(
      { error: `Zip code "${zipCode}" not found in Zillow data` },
      { status: 404 }
    )
  } catch (error) {
    console.error('Error in Zillow zip API:', error)
    return NextResponse.json(
